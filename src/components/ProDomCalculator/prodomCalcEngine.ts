import type { HouseCalcState, EstimateResult, EstimateSection, EstimateLineItem, HouseModelId } from '../HouseCalculator/houseCalcTypes';
import {
    HOUSE_MODELS,
    ENGINEERING_OPTIONS,
    FRAME_OPTIONS,
...
export function calculateHouseEstimate(state: HouseCalcState): EstimateResult {
    ...
    const baseFloorCredit = Math.round(700 * 1.12);
    const baseBathFloorCredit = Math.round(1500 * 1.12);
    if (state.floorFinish !== 'none') {
        const floorArea = model.area - (state.bathroomFloorArea || 0);
        if (floorArea > 0) {
            addFixed(baseItems, 'Вычет стандартного пола', -(floorArea * baseFloorCredit), true);
        }
    }
    if (state.bathroomFloorArea > 0 && state.bathroomFloorFinish !== 'none') {
        addFixed(baseItems, 'Вычет стандартного пола санузла', -(state.bathroomFloorArea * baseBathFloorCredit), true);
    }
    ...
    if (state.addFoundation) {
        const pilePrice = state.pileLength === '2500' ? Math.round(7000 * 1.12) : state.pileLength === '3000' ? Math.round(7800 * 1.12) : Math.round(8300 * 1.12);
    ...
    if (state.bathroomFloorArea > 0) {
        if (state.bathroomFloorFinish !== 'none') {
            const bathFloorItem = BATHROOM_PRICES.floor[state.bathroomFloorFinish as keyof typeof BATHROOM_PRICES.floor];
            addItem(bathItems, `Пол санузла — ${bathFloorItem.name}`, state.bathroomFloorArea, 'м²', bathFloorItem.price);
        }
        if (state.bathroomWallFinish !== 'none') {
            const bathWallArea = Math.round(Math.sqrt(state.bathroomFloorArea) * 4 * 3.25 * 10) / 10;
            const bathWallItem = BATHROOM_PRICES.wall[state.bathroomWallFinish as keyof typeof BATHROOM_PRICES.wall];
            const upgradePrice = bathWallItem.price;
            if (upgradePrice !== 0) addItem(bathItems, `Стены санузла — ${bathWallItem.name}`, bathWallArea, 'м²', upgradePrice);
        }
    }
    ...
    if (state.heatingSystem === 'electric') {
        addItem(engItems, 'Тёплый пол электрический модульный ЗЕБРА ЭВО-300 WF', state.warmFloorArea, 'м²', Math.round(2200 * 1.12));
