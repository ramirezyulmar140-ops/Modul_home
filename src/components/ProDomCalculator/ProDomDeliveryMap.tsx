...
import type { HouseCalcState, DeliveryVehicleEntry } from '../HouseCalculator/houseCalcTypes';
import { DELIVERY_VEHICLES } from './prodomCalculatorData';
...
const OVERSIZED_BASE_PRICE = Math.round(20000 * 1.12);
const CRANE_SERVICES_PRICE = Math.round(30000 * 1.12);

export const ProDomDeliveryMap: React.FC<DeliveryMapProps> = ({ state, onChange }) => {
                        ...
                        <YMaps query={{ apikey: '341481f1-976f-4208-893b-868e5f953b10', load: 'package.full', lang: 'ru_RU' }}>
                            <Map
                                ...
                                onLoad={(ymaps: unknown) => {
                                    ...
                                    if (document.getElementById('prodom-suggest-input') && !document.getElementById('prodom-suggest-input-ready')) {
                                        const suggestView = new y.SuggestView('prodom-suggest-input');
                                        const hiddenMarker = document.createElement('div');
                                        hiddenMarker.id = 'prodom-suggest-input-ready';
